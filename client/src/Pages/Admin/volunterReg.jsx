...
const FacultyList = () => {
  ...
  const fetchFacultyData = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(url+'/api/admin/getvolunterReg');
...
{store.admin.isAuthenticated ? (
    ...
    <div style={{ width: '97%', marginLeft: '1.5%' }}>
      <h2 style={{ textAlign: 'center',marginTop:'15px' }}>Volunteer Register List</h2>
      {facultyList.length === 0 ? (
        <p style={{ textAlign: 'center' }}>Loading Volunteer data...</p>
      ) : (
